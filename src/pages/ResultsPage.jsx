...
import { Trophy, Medal, BarChart, Timer, ArrowLeft, Home } from 'lucide-react';
import { useQuiz } from '../context/QuizContext';

const ResultsPage = () => {
  const navigate = useNavigate();
  const { state, dispatch } = useQuiz();
  const { score, answers, selectedCategory, gameOver } = state;
  const [activeTab, setActiveTab] = useState('summary');
  
  useEffect(() => {
    // If no game results to show, redirect to home
    if (!gameOver || !selectedCategory || answers.length === 0) {
      navigate('/');
    }
  }, [gameOver, selectedCategory, answers, navigate]);
  
  if (!selectedCategory || answers.length === 0) {
    return null;
  }
  ...
  const handlePlayAgain = () => {
    ...
    navigate(`/quiz-setup/${selectedCategory.id}`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4 justify-center pt-4">
        <button 
          ...
          className="bg-purple-600 text-white rounded-lg px-6 py-3 transition-all hover:bg-purple-700 flex items-center"
        >
        ...
        <button 
          ...
          className="bg-gray-600 text-white rounded-lg px-6 py-3 transition-all hover:bg-gray-700 flex items-center"
        >
